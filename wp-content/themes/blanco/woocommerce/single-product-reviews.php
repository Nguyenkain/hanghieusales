...
<?php if ( comments_open() ) : ?><div id="reviews"><?php 
	...
	$comment_form = array(
		...
		'comment_notes_after' => '',
		'fields' => array(
			'author' => '<div class="formField">' . 
						'<label for="author">' . __( 'Name', 'woocommerce' ) . '<span class="required">*</span>' . '</label> ' .
			            '<input id="author" name="author" type="text" class="textField" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" /></p>' . 
			            '<div class="clear"></div>' . 
			            '</div>',
			'email'  => '<div class="formField">' . 
						'<label for="email">' . __( 'Email', 'woocommerce' ) . '<span class="required">*</span>' . '</label> ' . 
			            '<input id="email" name="email" type="text" class="textField" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" aria-required="true" /></p>' .
			            '<div class="clear"></div>' . 
			            '</div>',
		),
	...
	$comment_form['comment_field'] .= '
		...
		</p>' . $woocommerce->nonce_field('comment_rating', true, false);
