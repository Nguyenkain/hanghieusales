...
<div class="product-images">
  ...
  <?php if (wpsc_the_product_thumbnail()) : ?>
    ...
    <div class="main-image" style="position:relative;">
      <a href="<?php echo wpsc_the_product_image(); ?>"
         <?php if (etheme_get_option('cloud_zoom')): ?>class="cloud-zoom" <?php else: ?> rel="lightbox[gall]"<?php
        endif;
      ?> id="zoom1" cloud-zoom-data="adjustX: 10, adjustY:-4" style="position: relative; display: block; ">
        <img class="product_image" id="product_image_<?php echo $product_id; ?>"
             alt="<?php echo wpsc_the_product_title(); ?>" title="<?php echo wpsc_the_product_title(); ?>"
             src="<?php echo etheme_get_image(false, $big_width, $big_height, false) ?>"/>
      </a>
    ...
    if (count($attachments) > 1) {
      ...
      <div class="views-gallery">
        <ul
          class="slider <?php if (count($attachments) > 4 && $product_layout == 'universal') { ?>jcarousel-horizontal<?php } ?>">
          <?php
            ...
            foreach ($attachments as $id => $attachment) {
              ...
              <li class="slide">
                <a href="<?php echo $full_img ?>"
                   <?php if (etheme_get_option('cloud_zoom')): ?>class="cloud-zoom-gallery"<?php else: ?> rel="lightbox[gall]"<?php
                  endif;
                ?> cloud-zoom-data="useZoom: 'zoom1', smallImage: '<?php echo $big_img ?>'">
  ...
  <?php else: ?>
    <div class="main-image" style="position:relative;">
      <a class="no-image" id="zoom1" cloud-zoom-data="adjustX: 10, adjustY:-4"
         style="position: relative; display: block; ">
        <img class="no-image" id="product_image_<?php echo $product_id; ?>" alt="No Image"
  ...
  <div class="price-block">
    ...
    <?php if (!wpsc_product_is_donation()) : ?>
      <?php if (wpsc_product_on_special()) : ?>
        <span class="old-price <?php echo $product_id; ?>"><?php _e('Old Price', ETHEME_DOMAIN); ?>: <span
            class="price"
            id="old_product_price_<?php echo $product_id; ?>"><?php echo wpsc_product_normal_price(); ?></span></span>
        <br/>
      <?php endif; ?>
      <span class="onsale-price <?php echo $product_id; ?>"><?php _e('Price', ETHEME_DOMAIN); ?>: <span
          class="price currentprice pricedisplay"
          id='product_price_<?php echo $product_id; ?>'><?php echo wpsc_the_product_price(); ?></span></span><br/>

      <?php if (wpsc_product_on_special()) : ?>
        <span class="you-save product_<?php echo $product_id; ?>"><?php _e('You save', ETHEME_DOMAIN); ?>: <span
            class="price"
            id="yousave_<?php echo $product_id; ?>"><?php echo wpsc_currency_display(wpsc_you_save('type=amount'), array('html' => false)); ?>
            (<?php echo wpsc_you_save(); ?>%)</span></span>
      ...
      <?php if (wpsc_show_pnp()) : ?>
        <p class="pricedisplay ppp_price"><?php _e('Shipping', ETHEME_DOMAIN); ?>:<span
            class="pp_price"><?php echo wpsc_product_postage_and_packaging(); ?></span></p>
      <?php endif; ?>

    <?php endif; ?>
  </div>
  <div class="product-stock">
      <span class="product-code"><?php _e('Product code', ETHEME_DOMAIN); ?>
        : <span><?php echo $product_id; ?></span></span><br/>
    <?php if (wpsc_show_stock_availability()): ?>
      <?php if (wpsc_product_has_stock()) : ?>
        <span class="stock in-stock"><?php _e('Availability', ETHEME_DOMAIN); ?>
          : <span><?php _e('in stock', ETHEME_DOMAIN); ?></span></span>
      <?php else: ?>
        <span class="stock out-stock"><?php _e('Availability', ETHEME_DOMAIN); ?>
          : <span><?php _e('out of stock', ETHEME_DOMAIN); ?></span></span>
      <?php endif; ?>
    <?php endif; ?>
  </div>
  <div class="clear"></div>
...
<div class="product-description">
  ...
  <?php if (etheme_get_custom_field('_etheme_size_guide')) : ?>
    <div class="size_guide">
      <a rel="lightbox"
         href="<?php etheme_option('size_guide_img'); ?>"><?php _e('Sizing Guide', ETHEME_DOMAIN); ?></a>
    </div>
  <?php endif; ?>
  <div class="clear"></div>
</div>
...
<form class="product_form" enctype="multipart/form-data" action="<?php echo wpsc_this_page_url(); ?>" method="post"
      name="1" id="product_<?php echo $product_id; ?>">
  ...
  <?php if (wpsc_product_is_donation()) : ?>
    ...
    <input type="text" id="donation_price_<?php echo $product_id; ?>" name="donation_price"
           value="<?php echo wpsc_calculate_price($product_id); ?>" size="6"/>
  <?php endif; ?>
  ...
  <div class="addto-container">
    ...
    <?php if (wpsc_has_multi_adding()): ?>
      <div class="qty-block">
        ...
        <input type="text" class="qty-input" id="wpsc_quantity_update_<?php echo $product_id; ?>"
      ...
      <?php if (wpsc_product_has_stock()) : ?>
        ...
        <?php if (wpsc_product_external_link($product_id) != '') : ?>
          ...
          <button class="button big active"
                  onclick="return gotoexternallink('<?php echo $action; ?>', '<?php echo wpsc_product_external_link_target($product_id); ?>')">
            <span><?php echo wpsc_product_external_link_text($product_id, __('Buy Now', ETHEME_DOMAIN)); ?></span>
          </button>
        <?php else: ?>
        
          <button type="submit" name="Buy" class="button big active wpsc_buy_button"
                  id="product_<?php echo $product_id; ?>_submit_button">
            <span><?php _e('Add To Cart', ETHEME_DOMAIN); ?></span></button>
...
<div class="product-sidebar">
  ...
  <?php if (wpsc_show_fb_like()): ?>
    ...
    <iframe
      src="https://www.facebook.com/plugins/like.php?href=<?php echo wpsc_the_product_permalink(); ?>&amp;layout=standard&amp;show_faces=true&amp;width=140&amp;action=like&amp;font=arial&amp;colorscheme=light"
...
<?php $relateds = get_related_products($product_id, true, 220, 220);
  $rand = rand(10, 1000); ?>
<?php if (count($relateds) > 0): ?>
  <div class="product-slider related">
    ...
    <div class="carousel">
      <div class="slider">
        <?php $_i = 0;
          foreach ($relateds as $product): $_i++; ?>
            <div class="slide product-slide <?php if (count($relateds) < 5 && $_i == 4) echo 'last' ?>">
              <?php etheme_product_labels($product['id']); ?>
              <a href="<?php echo $product['permalink'] ?>" class="product-image"><img alt=""
                                                                                       src="<?php echo $product['image'] ?>"/></a>
              <span class="product-name"><a
                  href="<?php echo $product['permalink'] ?>"><?php echo $product['title'] ?></a></span>

              <?php if ($product['normal_price'] != $product['price']): ?>
                <div class="price sale">
                  <p class="oldprice-p pricedisplay"><span
                      class="oldprice"><?php echo $product['normal_price'] ?></span></p>

                  <p class="pricedisplay"><span class="currentprice pricedisplay"><?php echo $product['price'] ?></span>
                  </p>
                </div>
              <?php else: ?>
                <div class="price">
              ...
              <div class="btn-cont">
                <a href="<?php echo $product['permalink'] ?>"
                   class="button add-to-cart sml"><span><?php _e('Read more', ETHEME_DOMAIN); ?></span></a>
              </div>
            </div>
          <?php endforeach; ?>
      </div>
...
<?php $crosss = etheme_products_also_bought($product_id, 220, 220);
  $rand = rand(10, 1000); ?>
...
<form onsubmit="submitform(this);return false;" action="<?php echo wpsc_this_page_url(); ?>" method="post"
      name="product_<?php echo $product_id; ?>" id="product_extra_<?php echo $product_id; ?>">
