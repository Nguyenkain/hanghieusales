    ...
    <div class="views-gallery">
        <ul class="slider <?php if (count($attachment_ids) > 3 && $product_layout == 'universal'){ ?>jcarousel-horizontal<?php } ?>">
            <?php if (count($attachment_ids) > 0 && $cloud_zoom == 1): ?>
	            <li class="slide">
		            <a href="<?php echo wp_get_attachment_url( get_post_thumbnail_id() ); ?>" class="lightbox cloud-zoom-gallery image" cloud-zoom-data="useZoom: 'zoom1', smallImage: '<?php echo etheme_get_image( $attachment->ID, $mainWidth, $mainHeight, $crop ) ?>'">
            ...
            <?php
                ...
                foreach ( $attachment_ids as $id ) {
                    ...
                    ?>
                    <li class="slide">
                        <a href="<?php echo wp_get_attachment_url( $id ) ?>" class="cloud-zoom-gallery image" <?php if($cloud_zoom == 1){ ?>cloud-zoom-data="useZoom: 'zoom1', smallImage: '<?php echo etheme_get_image( $id, $mainWidth, $mainHeight, $crop ) ?>'"<?php }else{ ?> rel="lightbox[gallery]" <?php } ?>>
