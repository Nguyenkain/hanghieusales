...
<div id="default_products_page_container"
     class="wrap wpsc_container <?php if (!$product_sidebar) echo 'no-sidebar'; else echo 'with-sidebar' ?>">
  ...
  <div class="wpsc_categories">
    ...
    <div class="wpsc_category">
      <a href="<?php wpsc_print_category_url(); ?>"
         class="wpsc_category_link <?php wpsc_print_category_classes_section(); ?>"
         title="<?php wpsc_print_category_name(); ?>"><?php wpsc_print_category_name(); ?></a>
  ...
  <div class="toolbar">
    ...
    <?php if ($view_mode == 'grid_list'): ?>
      <p class="view-mode">
        ...
        <span
          class="switcher switchToList <?php if ($view_mode_default == 'list') echo 'default_mode' ?>"><?php _e('List', ETHEME_DOMAIN); ?></span>
        <span
          class="switcher switchToGrid <?php if ($view_mode_default == 'grid') echo 'default_mode' ?>"><?php _e('Grid', ETHEME_DOMAIN); ?></span>
      </p>
    <?php endif; ?>
    <div class="clear"></div>
  </div>
...
<?php
  if ($view_mode == 'grid_list') {
    ...
    $view_class = ($view_mode == 'list') ? 'products_list' : 'products_grid';
  }
?>
<?php $columns = $product_per_row;
  $_i = 0; ?>
  <div id="products-grid" class="<?php echo $view_class; ?> rows-count<?php echo $columns ?>">
    ...
    <?php while (wpsc_have_products()) :  wpsc_the_product(); ?>
      <?php $_i++; ?>
      <div
        class="<?php if ($_i % $columns == 0) echo 'last' ?>  product-grid product_view_<?php echo wpsc_the_product_id(); ?> <?php echo wpsc_category_class(); ?> group">
        ...
        <?php if (wpsc_show_thumbnails()) : ?>
          <div class="imagecol" id="imagecol_<?php echo wpsc_the_product_id(); ?>">
            ...
            <?php if (wpsc_the_product_thumbnail()) :
              ?>
              <a id="<?php echo etheme_get_image($id, 400, 400, false) ?>"
                 href="<?php echo wpsc_the_product_permalink(); ?>"
                ...
                <?php if (etheme_get_custom_field('_etheme_hover') && $product_img_hover == 'swap'): ?>
                  <div class="img-wrapper"><img class="product_image" alt="<?php echo wpsc_the_product_title(); ?>"
                                                title="<?php echo wpsc_the_product_title(); ?>"
                                                src="<?php echo etheme_get_custom_field('_etheme_hover'); ?>"/>
                  </div> <?php endif; ?>
                <div
                  class="img-wrapper <?php if (etheme_get_custom_field('_etheme_hover') && $product_img_hover == 'swap'): ?> hideableHover <?php endif; ?>"
                  style="opacity: 1;"><img class="product_image" alt="<?php echo wpsc_the_product_title(); ?>"
                                           title="<?php echo wpsc_the_product_title(); ?>"
                                           src="<?php echo wpsc_the_product_thumbnail(); ?>"/></div>
              </a>

            <?php else: ?>
              <a class="product-image" href="<?php echo wpsc_the_product_permalink(); ?>">
                <div
                  class="img-wrapper<?php if (etheme_get_custom_field('_etheme_hover') && $product_img_hover == 'swap') echo ' hideableHover' ?>">
                  <img class="no-image" id="product_image_<?php echo wpsc_the_product_id(); ?>" alt="No Image"
                       title="<?php echo wpsc_the_product_title(); ?>"
                       src="<?php echo WPSC_CORE_THEME_URL; ?>wpsc-images/noimage.png"
                       width="<?php echo get_option('product_image_width'); ?>"
                       height="<?php echo get_option('product_image_height'); ?>"/></div>
              </a>
            <?php endif; ?>
            <?php
        ...
        <?php endif; ?>
        <?php if (etheme_get_option('_etheme_custom_brand')): ?>
          <div class="product-brand">
            <a class="wpsc_product_title"
               href="<?php echo wpsc_the_product_permalink(); ?>"><?php echo etheme_get_option('_etheme_custom_brand'); ?></a>
          </div>
        <?php else : ?>
          <div class="product-brand">
            <a class="wpsc_product_title"
               href="<?php echo wpsc_the_product_permalink(); ?>"><?php echo "No Brand"; ?></a>
          </div>
        <?php endif; ?>
        <?php if (etheme_get_option('product_page_productname')): ?>
          <div class="product-name">
            ...
            <?php else: ?>
              <a class="wpsc_product_title"
                 href="<?php echo wpsc_the_product_permalink(); ?>"><?php echo wpsc_the_product_title(); ?></a>
            <?php endif; ?>
          </div>
        <?php endif; ?>
        <?php
        ...
        <?php endif; ?>
        <form class="product_form" enctype="multipart/form-data" action="<?php echo $action; ?>" method="post"
              name="product_<?php echo wpsc_the_product_id(); ?>" id="product_<?php echo wpsc_the_product_id(); ?>">
          ...
          <?php if (etheme_get_option('product_page_price')): ?>
            <div class="price">
              <?php if (wpsc_product_is_donation()) : ?>
                <label for="donation_price_<?php echo wpsc_the_product_id(); ?>"><?php _e('Donation', ETHEME_DOMAIN); ?>
                  : </label>
                <input type="text" id="donation_price_<?php echo wpsc_the_product_id(); ?>" name="donation_price"
                       value="<?php echo wpsc_calculate_price(wpsc_the_product_id()); ?>" size="6"/>

              <?php else : ?>
                <?php if (wpsc_product_on_special()) : ?>
                  <p class="oldprice-p pricedisplay product_<?php echo wpsc_the_product_id(); ?>"><span class="oldprice"
                                                                                                        id="old_product_price_<?php echo wpsc_the_product_id(); ?>"><?php echo wpsc_product_normal_price(); ?></span>
                  </p>
                <?php endif; ?>
                <p class="pricedisplay product_<?php echo wpsc_the_product_id(); ?>"><span
                    id='product_price_<?php echo wpsc_the_product_id(); ?>'
                    class="<?php if (wpsc_product_on_special()) echo 'currentprice'; ?> pricedisplay"><?php echo wpsc_the_product_price(); ?></span>
                </p>
                <?php if (wpsc_product_on_special()) : ?>
                ...
                <div class="btn-cont">
                  <?php if (wpsc_product_external_link(wpsc_the_product_id()) != '') : ?>
                    ...
                    <button class="button" type="submit"
                            onclick="return gotoexternallink('<?php echo $action; ?>', '<?php echo wpsc_product_external_link_target(wpsc_the_product_id()); ?>')">
                      <span><?php echo wpsc_product_external_link_text(wpsc_the_product_id(), __('Buy Now', ETHEME_DOMAIN)); ?></span>
                    </button>

                  <?php else: ?>
                    <?php if (wpsc_have_variation_groups()) { ?>
                      <a href="<?php echo wpsc_the_product_permalink(); ?>"
                         class="button"><span><?php _e('Read More', ETHEME_DOMAIN); ?></span></a>
                    <?php } else { ?>
                      <button class="button" type="submit"
